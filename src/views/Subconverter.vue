    ...
    confirmLoadConfig() {
      ...
      (async () => {
        ...
        if (param.get("append_type")) {
          this.form.append类型 = param.get("append_type") === 'true';
        }
        if (param.get("tls13")) {
    ...
    getBackendVersion() {
      this.$axios
          ...
          .then(res => {
            ...
            a ? this.$message.success(`${this.backendVersion}`) : b ? this.$message.success(`${this.backendVersion}` + "本地局域网自建版后端") : this.$message.success(`${this.backendVersion}` + "官方原版后端不支持vless/hysteria订阅转换");
          })
          .catch(() => {
